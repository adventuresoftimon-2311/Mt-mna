...
export function renderAdmin() {
      ...
      <section class="admin-login fade-in" style="min-height: 100vh; display: flex; align-items: center; justify-content: center; background-color: var(--primary-bg);">
        <div class="card" style="width: 100%; max-width: 400px; text-align: center;">
          ...
          <form id="admin-login-form" style="display: flex; flex-direction: column; gap: 1rem;">
            <input type="text" id="admin-user" placeholder="Username" style="padding: 1rem; border: 1px solid var(--divider); background: var(--secondary-bg); color: var(--text-primary); font-size: 1rem; border-radius: 0; outline: none;" required>
            <input type="password" id="admin-pass" placeholder="Password" style="padding: 1rem; border: 1px solid var(--divider); background: var(--secondary-bg); color: var(--text-primary); font-size: 1rem; border-radius: 0; outline: none;" required>
            <button type="submit" style="padding: 1rem; background: var(--text-primary); color: var(--primary-bg); border: none; font-size: 1rem; font-weight: 600; text-transform: uppercase; cursor: pointer; transition: opacity 0.3s; margin-top: 1rem; border-radius: 0;">Authenticate</button>
  ...
  const postsHtml = posts.map(post => `
    <div style="display: flex; justify-content: space-between; align-items: center; padding: 1.5rem; border: 1px solid var(--divider); margin-bottom: 1rem; background: var(--card-bg);">
      ...
      <button class="delete-post-btn" data-id="${post.id}" style="padding: 0.5rem 1rem; background: #ef4444; color: white; border: none; cursor: pointer; border-radius: 0; text-transform: uppercase; font-size: 0.8rem; font-weight: 600;">Delete</button>
    </div>
  `).join('');
    ...
    <section class="admin-dashboard fade-in" style="min-height: 100vh; padding: 15vh 0; background-color: var(--secondary-bg);">
      <div class="container" style="max-width: 800px;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 3rem;">
          <h2>Portal Admin</h2>
          <button id="admin-logout-btn" style="padding: 0.5rem 1rem; background: transparent; border: 1px solid var(--text-primary); color: var(--text-primary); cursor: pointer; border-radius: 0; text-transform: uppercase; font-weight: 600; font-size: 0.8rem;">Disconnect</button>
        </div>
        ...
        <div class="card" style="margin-bottom: 3rem;">
          ...
          <form id="add-post-form" style="display: flex; flex-direction: column; gap: 1rem;">
            <input type="text" id="post-title" placeholder="Article Title" style="padding: 1rem; border: 1px solid var(--divider); background: var(--primary-bg); color: var(--text-primary); border-radius: 0; outline: none;" required>
            <input type="url" id="post-video" placeholder="Background Video URL (MP4)" style="padding: 1rem; border: 1px solid var(--divider); background: var(--primary-bg); color: var(--text-primary); border-radius: 0; outline: none;" required>
            <textarea id="post-content" placeholder="Content Body..." rows="5" style="padding: 1rem; border: 1px solid var(--divider); background: var(--primary-bg); color: var(--text-primary); border-radius: 0; outline: none; resize: vertical;" required></textarea>
            <button type="submit" style="padding: 1rem; background: var(--text-primary); color: var(--primary-bg); border: none; font-size: 1rem; font-weight: 600; text-transform: uppercase; cursor: pointer; transition: opacity 0.3s; margin-top: 1rem; border-radius: 0;">Publish Article</button>
          </form>
        </div>
