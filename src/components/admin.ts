...
export function renderAdmin() {
  ...
  const postsHtml = posts.map(post => `
    <div style="display: flex; justify-content: space-between; align-items: center; padding: 1.5rem; border: 1px solid var(--divider); margin-bottom: 1rem; background: var(--card-bg);">
      ...
      <button class="delete-post-btn" data-id="${post.id}" style="padding: 0.5rem 1rem; background: #ef4444; color: white; border: none; cursor: pointer; border-radius: 12px; text-transform: uppercase; font-size: 0.8rem; font-weight: 600;">Delete</button>
    </div>
  `).join('');
